...
Note that the 2048 game deploys a `NodePort` service, not a `LoadBalancer` one. The Application Load Balancer *is* the ingress controller, thus there is no separate ingress controller workload (like `ingress-nginx`) as it isn't necessary. The ingress logic is done inside the ALB via listeners and target groups, and the ALB binds directly to the service node port on all the cluster nodes.
